...
$: onMessageClick = chatHelper('onMessageClick', {onVideoLinkClick});
...
<InfinityScroll class="main is-above-chat-input" on:scrolled="{onInfinityScrolled}" on:visibility="{onInfinityVisibility}">
  ...
  {#each $messages.render() as message, i}
    ...
    <div class="{message.className}" class:is-not-present="{!$conversation.findParticipant(message.fromId)}" class:show-details="{!!message.showDetails}" data-index="{i}" data-ts="{message.ts.toISOString()}" on:click="{onMessageClick}">
